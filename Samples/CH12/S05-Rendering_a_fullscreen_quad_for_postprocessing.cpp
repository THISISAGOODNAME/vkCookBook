...
class Sample : public VulkanCookbookSample {
    ...
    virtual bool Initialize( WindowParameters WindowParameters ) override {
        ...
        if( !GetBinaryFileContents( "Data/Shaders/12 Advanced Rendering Techniques/05 Rendering a fullscreen quad for postprocessing/shader.vert.spv", vertex_shader_spirv ) ) {
            return false;
        }
        ...
        if( !GetBinaryFileContents( "Data/Shaders/12 Advanced Rendering Techniques/05 Rendering a fullscreen quad for postprocessing/shader.frag.spv", fragment_shader_spirv ) ) {
            return false;
        }
        ...
        SpecifyPipelineRasterizationState( false, false, VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE, VK_FRONT_FACE_COUNTER_CLOCKWISE, false, 0.0f, 1.0f, 0.0f, 1.0f, rasterization_state_create_info );
...
VULKAN_COOKBOOK_SAMPLE_FRAMEWORK( "12/05 - Rendering a fullscreen quad for postprocessing", 50, 25, 800, 600, Sample )
